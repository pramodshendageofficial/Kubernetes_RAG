...
BATCH_SIZE = 50
_GEMINI_DIM = 3072  #primary embedding model (gemini-embedding-2-preview)
_FALLBACK_DIM = 768  # all-mpnet-base-v2  - fallback embedding model
...
def _init():
    ...
    gemini = _probe_gemini()    #check if Gemini is available and working
    if gemini:
        _active_model = gemini
        _model_type = "gemini"
    else:
        _active_model = _load_fallback()    #load fallback model if Gemini is not available
...
def _embed_batch(batch: list[str]) -> list[list[float]]:
    ...
    Raises on other errors or if rate limit persists after 4 attempts.
    '''
...
def embed_texts(texts: list[str]) -> list[list[float]]:
    '''
    Embed a list of texts in batches. Returns a list of vectors.
    '''
    _init()
    ...
    print(f"✅ Embedded {len(all_embeddings)} texts using {_model_type} model. Embedding dim: {len(all_embeddings[0])}.")
